 ...
 * ML pipeline  spark-shell --packages com.databricks:spark-csv_2.11:1.4.0
 */
object SantanderLogisticRegression {
  def main(args: Array[String]): Unit = {
    val spark = SparkSession.builder.master("local[8]").appName("Document Similarity").getOrCreate()
    val sc = spark.sparkContext
    // Load train data as a dataframe
    val rawData = sqlContext.read.format("com.databricks.spark.csv").option("header", "true").option("inferSchema", "true").load("santander/dataset/train.csv")
    ...
    val joineddf = firstdf.unionAll(cdf)
    ...
    val gbt = new GBTClassifier().setLabelCol("label").setFeaturesCol("features").setMaxIter(30)
    ...
    // Fit against best model
    //val cvModel = cv.fit(training)
    // Save model
    cvModel.save("/Users/jshetty/Documents/kaggle/santander/model/mlpipeline/GBTModel1")
    ...
    // For submission
    val rawTest = sqlContext.read.format("com.databricks.spark.csv").option("header", "true").option("inferSchema", "true").load("/Users/jshetty/Documents/kaggle/santander/dataset/test.csv")
    ...
    finaldf.rdd.saveAsTextFile("santander/submission/ml_gbt1")
